...
def _real_writer(path, prompt):
    ...
    """
    try:
        import google_flow_sdk  # 실 백엔드(선택 설치). 미설치 시 ImportError.
    except ImportError as e:
        raise RuntimeError(
            "실 Google Flow 백엔드가 연결되어 있지 않다. "
            f"({_REAL_ENV}=1 옵트인했으나 SDK/API/MCP 미연결)"
        ) from e
    google_flow_sdk.generate_video(prompt=prompt, out_path=path)  # pragma: no cover
...
def generate(request, output_dir=None, writer=None, real=None):
    ...
    실 실행은 옵트인 전용이다. real=True 또는 환경변수 GOOGLE_FLOW_REAL=1 일 때만 실 백엔드를 시도한다.
