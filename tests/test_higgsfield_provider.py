# higgsfield_provider.generate 의 mock 영상 생성·SDK result 준수·검증·하네스 통과를 확인하는 테스트
"""Higgsfield Provider 단위 테스트. 결과는 schemas/higgsfield_result_schema.json + SDK result 로 검증."""
...
from higgsfield_provider import generate, PROVIDER, CAPABILITY  # noqa: E402
from provider_sdk import make_request  # noqa: E402
from provider_harness import run_harness  # noqa: E402

from jsonschema import Draft7Validator  # noqa: E402

_SCHEMA_PATH = os.path.join(_ROOT, "schemas", "higgsfield_result_schema.json")
...
def _assert_valid(r):
    ...
    assert not errs, "higgsfield 스키마 위반: " + "; ".join(e.message for e in errs)
...
def test_no_other_provider_imports():
    import higgsfield_provider as HF
    src = open(HF.__file__, encoding="utf-8").read()
    for forbidden in ("Google Flow", "Nano Banana", "Magnific", "Edge TTS", "subprocess", "ffmpeg"):
        assert forbidden not in src, f"금지 참조 발견: {forbidden}"


# --- 실 실행 옵트인 (Phase 6). 기본 mock 유지, 실 백엔드 미설치 시 graceful fail ---

from higgsfield_provider import _REAL_ENV  # noqa: E402
